...
def work(elc, audit_client, pad, community_etherpad):
    ...
    if pad_content_length <= 1500:
        is_ok = audit_client.check_content_ok(pad_content)
        if not is_ok:
            logger.info("find the result is:{}".format(pad))
            return get_reject_describe(pad, community_etherpad, pad_content), empty_pad_name
    else:
        ...
        for i in range(0, pad_content_length, 1500):
            ...
            is_ok = audit_client.check_content_ok(pad_content[start_index: end_index])
            if not is_ok:
                logger.info("find the result is:{}".format(pad))
                return get_reject_describe(pad, community_etherpad, pad_content), empty_pad_name
